...
class RunningDataProvider {
    ...
    companion object {
        ...
        private var channelObserver: ContentObserver =
            object : ContentObserver(Handler(Looper.getMainLooper())) {
                ...
                override fun onChange(selfChange: Boolean) {
                    ...
                    if (activeTvChannel != null) {
                        ...
                        checkLockedStatus(activeTvChannel)
                    } else {
        ...
        private fun notRunningBanner() {
            ...
            Log.i(
                TAG,
                "notRunningBanner: Sending event ${ReferenceEvents.ACTIVE_SERVICE_NOT_RUNNING}"
            )
            InformationBus.submitEvent(
                Event(
        ...
        private fun replacementBanner() {
            ...
            Log.i(
                TAG,
                "notRunningBanner: Sending event ${ReferenceEvents.ACTIVE_SERVICE_REPLACEABLE}"
            )
            InformationBus.submitEvent(
                Event(
        ...
        private fun replaceService(replaceService: String, lastChannel: String) {
            ...
            Log.i(
                TAG,
                "replaceService: ONID = ${tripletIDs[0]}, TSID = ${tripletIDs[1]}, ServiceID = ${tripletIDs[2]}"
            )
            ...
            var replaceChannel =
                (ReferenceSdk.dataProvider as TifDataProvider).searchChannelByTriplet(
                    replaceONId,
                    replaceTSId,
                    replaceServiceId
                )
                ...
                (ReferenceSdk.tvHandler as ReferenceTvHandler).changeChannel(
                    ...
                    object : AsyncReceiver {
                        ...
                        override fun onSuccess() {
                            ...
                            Toast.makeText(
                                context,
                                "Service $lastChannel not running, switching to ${replaceChannel.name}",
                                Toast.LENGTH_SHORT
                            ).show()
                            checkRunningStatus(replaceChannel)
                            checkLockedStatus(replaceChannel)
                            //updating active channel and its index, to resolve ChannelList and InfoBanner mismatch during Replacement Channel
                            (ReferenceSdk.tvHandler as ReferenceTvHandler).activeChannel = replaceChannel
                            (ReferenceSdk.tvHandler as ReferenceTvHandler).updateDesiredChannelIndex()
                            InformationBus.submitEvent(Event(ReferenceEvents.UPDATE_INFO_BANNER, replaceChannel))
                        }
                    })
        ...
        fun checkLockedStatus(activeTvChannel: ReferenceTvChannel?) {
            if (state != State.NOT_RUNNING) {
                if (activeTvChannel!!.isLocked && !(ReferenceSdk.playerHandler as ReferencePlayerHandler).isChannelUnlocked) {
        ...
        fun checkRunningStatus(activeTvChannel: ReferenceTvChannel?) {
            ...
            if (activeTvChannel != null) {
                ...
                val systemData = getRunningStatus(
                    activeTvChannel!!.onId,
                    activeTvChannel.tsId,
                    activeTvChannel.serviceId
                )
                if (systemData == null) {
        ...
        private fun getRunningStatus(onId: Int, tsId: Int, serviceId: Int): RunningData? {
            ...
            if (cursor != null && cursor!!.count > 0) {
                ...
                if (cursor?.getBlob(cursor.getColumnIndex(ReferenceContract.Channels.INTERNAL_PROVIDER_DATA_COLUMN)) != null) {
                    var providerDataBlob =
                        cursor.getBlob(cursor.getColumnIndex(ReferenceContract.Channels.INTERNAL_PROVIDER_DATA_COLUMN))
                    try {
                    ...
                    try {
                        ...
                        Log.i(
                            TAG,
                            "getRunningStatus: REPLACEMENT SERVICE = ${retRunning.replacementService}"
                        )
                    } catch (e: Exception) {
                    ...
                    Log.i(
                        TAG,
                        "getSystemInfoData: cursor.getColumnIndex(TvContract.Channels.COLUMN_INTERNAL_PROVIDER_DATA)) == null"
                    )
                }
            }
            cursor!!.close()
