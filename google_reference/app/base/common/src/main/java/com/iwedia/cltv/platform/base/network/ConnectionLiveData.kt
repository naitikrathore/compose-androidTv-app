package com.iwedia.cltv.platform.base.network
...
class ConnectionLiveData(private val cm: ConnectivityManager, private val connectionCallback: (hasConnection: Boolean)->Unit) : LiveData<NetworkData>() {
    ...
    private fun addValidNetworks(network: Network) {
        val net = NetworkData.Network(cm.getNetworkCapabilities(network), cm.getLinkProperties(network))
    ...
    companion object { const val TAG = "NetworkHandler"}
}