...
import com.iwedia.cltv.platform.base.content_provider.RecordingsDataProvider
...
open class PvrInterfaceBaseImpl(
    ...
    protected var tvRecordingClient: TvRecordingClient? = null
    protected var tuneCallback: TuneCallback? = null
    private var stopRecordingCallback: StopRecordingCallback? = null
    protected var recordingInProgress: RecordingInProgress? = null
     ...
     */
    lateinit var pvrPlaybackView: TvView
    ...
    private var pvrState = PvrInterface.PvrSeekType.REGULAR_SEEK //todo this is probably not needed any more
    var pvrPlaybackPositionMs = 0L

    init {
        recordingsDataProvider = RecordingsDataProvider(context, tvInterfaceImpl, timeInterface, utilsInterface)
    }
     ...
     */
    protected fun createRecordingClient() {
    ...
    override fun startRecordingByChannel(tvChannel: TvChannel, callback: IAsyncCallback, isInfiniteRec: Boolean){
...
//                                    tvInterfaceImpl.activeChannel = tvChannel
     ...
     */
    protected fun triggerRecordingTimer(duration: Long, tvChannel: TvChannel) {
        pvrStopTimer = object : CountDownTimer(duration, 1000) {
            ...
            override fun onTick(millisUntilFinished: Long) {
                ...
                else if (utilsInterface.isUsbConnected() && !utilsInterface.isUsbFreeSpaceAvailable()) {
                    cancel()
                    stopRecordingWhenInterrupted(tvChannel)
                }
                ...
                if (durationInSeconds != 0L
                     ...
                     || durationInSeconds % 60 == 0L))    {  // after 1 minute updating every 1 minute
    ...
    private fun stopRecordingWhenInterrupted(tvChannel: TvChannel) {
        stopRecordingByChannel(tvChannel, object : IAsyncCallback {
            ...
            override fun onSuccess() {
                if (!utilsInterface.isUsbConnected()) {
                    InformationBus.informationBusEventListener?.submitEvent(Events.PVR_RECORDING_FINISHED)}
                else if(!utilsInterface.isUsbFreeSpaceAvailable()) {
    ...
    protected fun getActiveChannelScrambled(): Boolean {
    ...
    override fun getRecordingList(callback: IAsyncDataCallback<List<Recording>>) {
        if(recordingsDataProvider == null){
            ...
            CoroutineHelper.runCoroutine({
                callback.onReceive(recordingsDataProvider!!.getRecordings())
            })
        }
    }
    ...
    override fun getPlaybackPositionPercent(recording: Recording): Double{
        ...
        if (playbackPosition.contains(recording.id)) {
            percent = (playbackPosition[recording.id]!!.toDouble()*100)/recording.duration.toDouble()
        }
        return percent
    }
...
//        return ReferenceSdk.tvInputHandler!!.isChannelRecordable(channel.inputId)
    ...
    private var recordingCallback = object : TvRecordingClient.RecordingCallback() {
        ...
        //Because this is Android and here will received hidden SDK event
        fun onEvent(inputId : String ,  eventType : String, args : Bundle) {
            handleRecordingEvent(inputId, eventType, args)
        }
    }

    open fun handleRecordingEvent(inputId : String , eventType : String, args : Bundle) {
        //do nothing here, will be overridden
    }

    /**
     * Destroy tv recording client instance
     */
    protected fun destroyRecordingClient() {
        ...
        if (tvRecordingClient != null) {
            ...
            recordingInProgress = null
            setRecIndication(false)
            //todo needs features from other implementations or that part needs to be moved here
