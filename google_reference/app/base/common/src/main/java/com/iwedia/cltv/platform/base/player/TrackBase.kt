...
sealed class TrackBase(val trackInfo: TvTrackInfo): ITrack {
    ...
    class AudioTrack(val track: TvTrackInfo, val utilsInterface: UtilsInterface,
        ...
        override var isSps : Boolean = getIsSpsAudio()
        override var isAdSps : Boolean = getIsAdSpsAudio()
        override var apdText : String = getApdAudioText()
        private fun getApdAudioText(): String {
           return utilsInterface.getApdString(track)
        }

        private fun getIsAdSpsAudio(): Boolean {
          return utilsInterface.hasAdSpsAudio(track)
        }

        private fun getIsSpsAudio(): Boolean {
            return utilsInterface.hasSpsAudio(track)
        }
