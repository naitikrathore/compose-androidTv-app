...
import com.iwedia.cltv.platform.mal_service.common.SchedulerDataProvider
...
class SchedulerInterfaceImpl(
     context: Context,
    val serviceImpl: IServiceAPI,
    val epgInterface: EpgInterface,
    val watchlistInterface: WatchlistInterface) : SchedulerInterface {
        ...
        override fun scheduleRecording(
            ...
            Log.d(Constants.LogTag.CLTV_TAG + "VANDANA","add schedule recoridngg")
            ...
            if (findConflictedRecordings(scheduledRecording).isNotEmpty()) {
                ...
                InformationBus.informationBusEventListener.submitEvent(
                    Events.SCHEDULED_RECORDING_CONFLICT,
            ...
            if (timeBeforeStartProgramme > 0) {
                ...
                Log.d(Constants.LogTag.CLTV_TAG +
                    TAG,
                    "onReceive: Cannot schedule $timeBeforeStartProgramme ${scheduledRecording.scheduledDateStart}"
                )
                callback.onReceive(SchedulerInterface.ScheduleRecordingResult.SCHEDULE_RECORDING_ERROR)
            }
        }
                ...
                override fun run() {
                    if (recordings.contains(scheduledRecording)) {
                        ...
                        if (watchlistInterface.checkReminderConflict(
                            ...
                            Log.d(Constants.LogTag.CLTV_TAG +
                                TAG,
                                "run: EVENT SUBMITTED -> SCHEDULED_RECORDING_NOTIFICATION/scheduledRecording == $scheduledRecording"
                            )
                ...
                fun removeAndReschedule() {
                    removeAllScheduledRecording(scheduledRecording,
                        object : IAsyncCallback {
                            ...
                            override fun onSuccess() {
                                //Current time
                                val currentTime = serviceImpl.getCurrentTime()
        ...
        override fun scheduleWithDailyRepeat(
            ...
            val recordingStart =
                (scheduledRecordingPrevious.scheduledDateStart + (24 * 60 * 60 * 1000))
            ...
            val tvEvent = TvEvent.createNoInformationEvent(
                ...
                serviceImpl.getCurrentTimeByChannel(toServiceChannel(scheduledRecordingPrevious.tvChannel!!))
            )
        ...
        override fun getChannelById(channelId: Int): TvChannel? {
            val channelList = serviceImpl.getChannelList(ApplicationMode.DEFAULT.ordinal)
            for (i in 0 until channelList.size) {
                if (channelList[i].id == channelId) {
                    return fromServiceChannel(channelList[i])
                }
            }
            return null
        }
        ...
        override fun getEventId(
            tvChannel: TvChannel, eventId: Int, callback: IAsyncDataCallback<TvEvent>
        ) {
            ...
            val startTime: Long =
                serviceImpl.getCurrentTimeByChannel(toServiceChannel(tvChannel)) - TimeUnit.HOURS.toMillis(0)
            val endTime: Long =
                serviceImpl.getCurrentTimeByChannel(toServiceChannel(tvChannel)) + TimeUnit.HOURS.toMillis((24 * 8).toLong())
        ...
        override fun removeScheduledRecordingForDeletedChannels() {
            CoroutineHelper.runCoroutine({
                ...
                serviceImpl.getChannelList(ApplicationMode.DEFAULT.ordinal).forEach { tvChannel ->
