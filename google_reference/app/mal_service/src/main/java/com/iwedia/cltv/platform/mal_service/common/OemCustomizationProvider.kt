...
class OemCustomizationProvider constructor(
...
//        initLanguagesData(context)
    ...
    private fun getScanType(context: Context): String {
        val cursor: Cursor? =
            context.contentResolver.query(OEM_CUSTOMIZATION_URI, null, null, null, null)
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                return cursor.getString(cursor.getColumnIndex("scan_type"))
            }
        }
        return "free"
    }
...
//        cv.put(TELETEXT_ENABLE_COLUMN, readFromJson(TELETEXT_ENABLE_COLUMN, OEM).toInt())
        ...
        cv.put(
            THIRD_PARTY_INPUT_ENABLED_COLUMN,
            readFromJson(THIRD_PARTY_INPUT_ENABLED_COLUMN, OEM).toInt()
        )
        cv.put(
            TERRESTRIAL_SCAN_ENABLED_COLUMN,
            readFromJson(TERRESTRIAL_SCAN_ENABLED_COLUMN, OEM).toInt()
        )
        cv.put(CABLE_SCAN_ENABLED_COLUMN, readFromJson(CABLE_SCAN_ENABLED_COLUMN, OEM).toInt())
        cv.put(
            SATELLITE_SCAN_ENABLED_COLUMN,
            readFromJson(SATELLITE_SCAN_ENABLED_COLUMN, OEM).toInt()
        )
        ...
        cv.put(
            CLEAR_CHANNELS_SCAN_OPTION_ENABLED_COLUMN,
            readFromJson(CLEAR_CHANNELS_SCAN_OPTION_ENABLED_COLUMN, OEM).toInt()
        )
        cv.put(
            DELETE_THIRD_PARTY_INPUT_COLUMN,
            readFromJson(DELETE_THIRD_PARTY_INPUT_COLUMN, OEM).toInt()
        )
    ...
    private fun createCompanyLogo(context: Context): ByteArray {
        val bitM: Bitmap =
            BitmapFactory.decodeStream(context.classLoader.getResourceAsStream("assets/company_logo.png"))
    ...
    private fun initSystemInfoData(context: Context) {
        ...
        var cv = ContentValues()
        cv.put(
            RF_CHANNEL_NUMBER_ENABLED_COLUMN,
            readFromJson(RF_CHANNEL_NUMBER_ENABLED_COLUMN, SYSTEM_INFO).toInt()
        )
        cv.put(BER_ENABLED_COLUMN, readFromJson(BER_ENABLED_COLUMN, SYSTEM_INFO).toInt())
        cv.put(
            FREQUENCY_ENABLED_COLUMN,
            readFromJson(FREQUENCY_ENABLED_COLUMN, SYSTEM_INFO).toInt()
        )
        ...
        cv.put(
            SERVICEID_ENABLED_COLUMN,
            readFromJson(SERVICEID_ENABLED_COLUMN, SYSTEM_INFO).toInt()
        )
        cv.put(
            POSTVITERBI_ENABLED_COLUMN,
            readFromJson(POSTVITERBI_ENABLED_COLUMN, SYSTEM_INFO).toInt()
        )
        ...
        cv.put(
            NETWORKID_ENABLED_COLUMN,
            readFromJson(NETWORKID_ENABLED_COLUMN, SYSTEM_INFO).toInt()
        )
        cv.put(
            NETWORKNAME_ENABLED_COLUMN,
            readFromJson(NETWORKNAME_ENABLED_COLUMN, SYSTEM_INFO).toInt()
        )
        cv.put(
            BANDWIDTH_ENABLED_COLUMN,
            readFromJson(BANDWIDTH_ENABLED_COLUMN, SYSTEM_INFO).toInt()
        )
    ...
    private fun inputConfigurableKey(
        name: String,
        actionType: Int,
        description: String,
        contentResolver: ContentResolver
