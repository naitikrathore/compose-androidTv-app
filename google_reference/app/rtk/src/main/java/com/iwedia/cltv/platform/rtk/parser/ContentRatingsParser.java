package com.iwedia.cltv.platform.rtk.parser;
...
import static com.realtek.tv.RtkSettingConstants.TV_COUNTRY_INDEX;
...
public class ContentRatingsParser {
    ...
    private List<ContentRatingSystem> parse(
        ...
        if (domain.equals(mContext.getPackageName())) {
            domain = TV_COUNTRY_INDEX.split("_")[0];
        } else {
            return null;
        }
        ...
        for (int i = 0; i < parser.getAttributeCount(); i++) {
            ...
            switch (attr) {
                ...
                case ATTR_DESCRIPTION:
                    ...
                    Log.d(Constants.LogTag.CLTV_TAG + TAG, "DESCRIPTION" + des);
                    int rat = -1;
