...
class ClosedCaptionInterfaceImpl(private var context: Context, val utilsModule: UtilsInterface, playerInterface: PlayerInterface) :
    ...
    private var closedCaptionCallback: com.iwedia.cltv.platform.mk5.ClosedCaptionCallback? = null
    ...
    init {
        ...
        var retVal = tvSettingConfig?.getConfigValueInt(TVSettingConfig.SETUP_ANALOG_CAPTION) ?:  0
        globalCCset = getDefaultCCValues("caption_services") ?: 0
        tempString = MtkTvBanner.getInstance().caption ?: ""
        ccPosition = retVal

        var enabled = tvSettingConfig?.getConfigValueInt(TVSettingConfig.SETUP_CAPTION_DISPLAY)
        if (enabled != null) {
    ...
    override fun saveUserSelectedCCOptions(ccOptions: String, newValue: Int, isOtherInput: Boolean) {
        when (ccOptions) {
            "display_cc" -> {
                var oldValue = tvSettingConfig?.getConfigValueInt(TVSettingConfig.SETUP_CAPTION_DISPLAY)
                tvSettingConfig?.setConifg(
                    TVSettingConfig.SETUP_CAPTION_DISPLAY,
                    newValue
                )
                if((oldValue == 0) && (newValue == 1)) {
                    var currentCC = getDefaultCCValues("caption_services")
                    if(currentCC != null) {
    ...
    override fun initializeClosedCaption() {
        val closedCaptionCallback = com.iwedia.cltv.platform.mk5.ClosedCaptionCallback(this)
