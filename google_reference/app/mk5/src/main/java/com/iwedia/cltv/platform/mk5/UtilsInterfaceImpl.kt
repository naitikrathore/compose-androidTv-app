...
import com.iwedia.cltv.platform.mk5.language.LanguageMapperImpl
...
class UtilsInterfaceImpl(
    ...
    override fun getAudioType(callback: IAsyncDataCallback<Int>?) {
        callback?.onReceive(if(getAudioDescriptionState()) {
            1
        } else {
            0
        })
    ...
    override fun getCountryCode(): String {
        ...
        val countryCode = MtkTvConfig.getInstance().getCountry()

        if (countryCode.length == 2) return countryCode

        val iso2Country = countryISOMapping[countryCode]

        return if (iso2Country != null) iso2Country else super.getCountryCode()
    }
